...
import { Loader2, ClipboardList, ImageIcon, Mountain, Route, Car, Send } from 'lucide-react';
...
// Chargement dynamique et asynchrone des composants Leaflet (Désactivation stricte du SSR)
const MapContainer = dynamic(() => import('react-leaflet').then(m => m.MapContainer), { ssr: false });
const TileLayer = dynamic(() => import('react-leaflet').then(m => m.TileLayer), { ssr: false });
const MapComponent = dynamic(() => import('../MapComponent'), { ssr: false });

// Sous-composant interne pour gérer les clics sur la carte Leaflet sans casser le SSR
function TransportPolyline({ points, setPoints }: { points: [number, number][], setPoints: any }) {
  // Importation à la volée du hook pour éviter les erreurs de build côté serveur
...
// 📦 1. Conteneur principal du Formulaire isolé pour le bloc Suspense
function CreateEventForm() {
  ...
  useEffect(() => {
    ...
    fetch('/api/rando_balades_Toulouse')
      .then(res => res.json())
      .then(json => {
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <Card className="border-blue-100">
        ...
        <CardContent className="space-y-6 pt-6">
          <div className="h-[400px] w-full relative z-0 border rounded-lg overflow-hidden">
            ...
            <div className="absolute bottom-2 left-2 z-[1000] bg-white p-2 rounded shadow text-xs text-slate-900">
          ...
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 space-y-4">
            <Label className="font-bold flex items-center gap-2 text-slate-900"><Car size={16}/> CAPACITÉ TRANSPORT</Label>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-900 p-8 rounded-2xl shadow-2xl mt-10">
        ...
        <div className="flex flex-wrap gap-4 w-full md:w-auto justify-center">
          ...
          <Button variant="outline" className="border-slate-500 bg-transparent text-white hover:bg-slate-800 hover:text-white" onClick={() => handleSave(false)} disabled={saving}>
            Brouillon
          </Button>

          <Button className="bg-green-500 hover:bg-green-400 text-white px-10 font-extrabold h-12 shadow-lg shadow-green-500/20" onClick={() => handleSave(true)} disabled={saving}>
            {saving ? (
              <Loader2 className="animate-spin h-5 w-5" />
            ) : (
              <div className="flex items-center"><Send className="mr-2 h-5 w-5" />Publier la sortie</div>
            )}
          </Button>
        </div>
      </div>
...
// 👑 2. Export par défaut enveloppé dans le Suspense Boundary exigé par Next.js
export default function CreateEventPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 gap-3">
        <Loader2 className="animate-spin h-10 w-10 text-green-700" />
        <p className="text-sm font-medium text-slate-500">Chargement de l'éditeur de randonnées...</p>
      </div>
    }>
      <CreateEventForm />
    </Suspense>
