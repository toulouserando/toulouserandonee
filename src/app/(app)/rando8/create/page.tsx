...
// Chargement du MapComponent local de rando8 sans SSR
const MapComponent = dynamic(() => import('../MapComponent'), { 
  ssr: false,
  loading: () => <div className="h-full flex items-center justify-center bg-slate-100 font-mono italic text-slate-400">Chargement de la carte Randoligne...</div>
});

// 📦 Composant interne pour encapsuler la logique du formulaire et useSearchParams
function CreateEventFormRando8() {
  const router = useRouter();
  const searchParams = useSearchParams();
  
  const id = searchParams.get('id');
  const sourceParam = searchParams.get('source') || 'rando8'; 
  ...
  // RÉCUPÉRATION DE L'ITINÉRAIRE UNIQUE DEPUIS L'API RANDO8
  useEffect(() => {
    fetch('/api/rando8')
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data) && id) {
          ...
          if (found) {
            setFilteredLines([found]); // Le MapComponent attend un tableau
            setTitle(`Randonnée sur la ligne : ${found.nom}`);
      ...
      .catch(err => {
        console.error("Erreur lors du chargement de la ligne rando8:", err);
        setLoading(false);
      });
  }, [id]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8 bg-slate-50/40 min-h-screen">
      <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">
        Planifier une Sortie <span className="text-emerald-600">Randoligne</span>
      </h1>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="border-b bg-slate-100/50">
          <CardTitle className="flex items-center gap-2 text-lg text-slate-900">
            <ClipboardList className="w-5 h-5 text-emerald-600"/> Description de la randonnée
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-6 pt-6">
          <div className="grid gap-2">
            <Label htmlFor="event-title">Titre de l'évènement</Label>
            <Input id="event-title" value={title} onChange={e => setTitle(e.target.value)} placeholder="Ex: Randonnée sportive sur la crête" className="h-11 border-slate-200" />
          </div>
      ...
      {/* 🗺️ VISUALISATION CARTE RANDO8 */}
      <Card className="border-slate-200 shadow-sm overflow-hidden">
        <CardHeader className="bg-slate-900 text-slate-50">
          <CardTitle className="flex items-center gap-2 text-md">
            <Map size={18} className="text-emerald-400"/> Localisation de la ligne sélectionnée
          </CardTitle>
        </CardHeader>
        ...
        <CardContent className="p-6 grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <Input type="number" value={distance} onChange={e => setDistance(e.target.value)} className="pl-10 h-10 border-slate-200" />
            </div>
          </div>
      ...
      <Card className="border-slate-200 shadow-sm">
        ...
        <CardContent className="space-y-6 pt-6">
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-1"><Label>Lieu de Départ / RDV</Label><Input value={meetingPoint} onChange={e => setMeetingPoint(e.target.value)} placeholder="Parking, Gare..." className="border-slate-200" /></div>
          ...
          <div className="bg-slate-100/60 p-4 rounded-lg border border-slate-200 space-y-4">
            ...
            <div className="space-y-1">
              ...
              <Textarea placeholder="Frais de péage à prévoir, lieu de rdv pour le covoiturage..." value={transportNotes} onChange={e => setTransportNotes(e.target.value)} className="bg-white border-slate-200" />
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-900 p-8 rounded-2xl shadow-xl mt-10">
        <div className="space-y-1">
          <h4 className="text-xl font-bold text-emerald-400">Publier la sortie ?</h4>
        ...
        <div className="flex flex-wrap gap-4 w-full md:w-auto justify-center">
          ...
          <Button className="bg-emerald-600 hover:bg-emerald-700 text-white px-10 font-bold h-12 shadow-lg" onClick={() => handleSave(true)} disabled={saving}>
...
// 👑 Export par défaut enveloppé sous Suspense pour Next.js (Bailout de rendu statique)
export default function CreateEventPageRando8() {
  return (
    <Suspense fallback={
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 gap-3">
        <Loader2 className="animate-spin h-10 w-10 text-emerald-600" />
        <p className="text-sm font-medium text-slate-600 font-mono">Chargement des tracés de lignes et de la carte...</p>
      </div>
    }>
      <CreateEventFormRando8 />
    </Suspense>
  );
}