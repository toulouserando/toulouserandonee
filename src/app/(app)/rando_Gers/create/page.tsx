...
// Chargement du MapComponent local de rando9 sans SSR
const MapComponent = dynamic(() => import('../MapComponent'), { 
  ssr: false,
  loading: () => <div className="h-full flex items-center justify-center bg-slate-100 font-mono italic text-slate-400">Chargement de la carte du Gers...</div>
});

// 📦 Composant interne contenant la logique du formulaire
function CreateEventFormRando9() {
  const router = useRouter();
  const searchParams = useSearchParams();
  
  const id = searchParams.get('id');
  const sourceParam = searchParams.get('source') || 'rando_Gers'; 
  ...
  // RÉCUPÉRATION DE L'ITINÉRAIRE UNIQUE DEPUIS L'API RANDO9 (GERS)
  useEffect(() => {
    fetch('/api/rando_Gers')
      ...
      .then(data => {
        if (Array.isArray(data) && id) {
          ...
          if (found) {
            setFilteredCircuits([found]); // Le MapComponent attend un tableau (mappé sur la prop `circuits`)
            setTitle(`Randonnée : ${found.nom}`);
            setMeetingPoint(found.commune || "");
            
            // Si la longueur est fournie en mètres par l'API rando9, conversion automatique en km
            if (found.longueur) {
              const distanceKm = (parseFloat(found.longueur) / 1000).toFixed(1);
      ...
      .catch(err => {
        console.error("Erreur lors du chargement du circuit rando9:", err);
        setLoading(false);
      });
  }, [id]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8 bg-slate-50/40 min-h-screen">
      <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">
        Planifier une Sortie <span className="text-blue-600">Gers (32)</span>
      </h1>
      ...
      <Card className="border-slate-200 shadow-sm">
        ...
        <CardContent className="space-y-6 pt-6">
          <div className="grid gap-2">
            ...
            <Input id="event-title" value={title} onChange={e => setTitle(e.target.value)} placeholder="Ex: Randonnée dans les vallons du Gers" className="h-11 border-slate-200" />
          </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-900 p-8 rounded-2xl shadow-xl mt-10">
        ...
        <div className="flex flex-wrap gap-4 w-full md:w-auto justify-center">
          ...
          <Button className="bg-blue-600 hover:bg-blue-700 text-white px-10 font-bold h-12 shadow-lg" onClick={() => handleSave(true)} disabled={saving}>
...
// 👑 Export par défaut enveloppé sous un Suspense global pour satisfaire Next.js
export default function CreateEventPageRando9() {
  return (
    <Suspense fallback={
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 gap-3">
        <Loader2 className="animate-spin h-10 w-10 text-blue-600" />
        <p className="text-sm font-medium text-slate-600 font-mono">Chargement de la page de création rando9...</p>
      </div>
    }>
      <CreateEventFormRando9 />
    </Suspense>
  );
}