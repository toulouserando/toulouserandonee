...
import { Compass, CalendarPlus, Map as MapIcon } from 'lucide-react';
...
export default function CircuitsPage() {
  ...
  const [filtre, setFiltre] = useState('Gers (32)');
  ...
  // Gestion du changement d'onglet (réinitialise le focus de la carte)
  const handleFiltreChange = (dept: string) => {
    setFiltre(dept);
    setSelectedCircuitId('all');
  };

  // 1. Liste complète des circuits correspondant au département sélectionné
    ...
    <main className="p-6 max-w-6xl mx-auto font-sans space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Gers & Haute-Garonne — Tracés géographiques et points de repère.</p>
        </div>
      ...
      {/* Onglets de sélection des départements */}
      <div className="flex gap-4 border-b border-slate-200">
        {['Gers (32)', 'Haute-Garonne (31)'].map(dept => (
          <button
            key={dept}
            onClick={() => handleFiltreChange(dept)}
            className={`pb-2 px-4 text-sm font-medium transition-all relative top-[1px] ${
              filtre === dept 
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          return (
            ...
            >
              ...
              <div className="space-y-2 mt-5">
                ...
                <button 
                  onClick={() => {
                    router.push(`/rando10/create?id=${c.id}&source=rando10`);
                  }}
        ...
        {circuitsFiltrés.length === 0 && (
          <div className="col-span-full text-center py-12 text-slate-400 font-medium italic">
            Aucun circuit trouvé pour la zone {filtre}.
          </div>
        )}
      </div>
    </main>
