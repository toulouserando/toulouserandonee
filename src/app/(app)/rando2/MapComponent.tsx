...
function ChangeView({ data }: { data: any[] }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      data.forEach((item) => {
        if (item.geometry) {
          // Crée un calque Leaflet temporaire pour calculer les dimensions géographiques du tracé
...
export default function MapComponent({ selectedSource, data, customPoints, setCustomPoints }: MapComponentProps) {
  ...
  const getPolylineStyle = (feature: any) => {
    return {
  ...
  return (
    <div className="h-full w-full bg-slate-100">
      ...
      >
        ...
        {data.map((circuit) => {
          if (!circuit.geometry) return null;
          
          return (
            <GeoJSON 
              key={circuit.id}
              data={circuit.geometry}
