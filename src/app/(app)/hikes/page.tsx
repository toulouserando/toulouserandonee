...
import { MapPin, Clock, TrendingUp, Plus, Loader2, Compass } from "lucide-react";
...
export default function HikesListPage() {
  ...
  useEffect(() => {
    async function fetchHikes() {
      try {
        ...
        setErrorMessage(err.message || "Impossible de charger les circuits.");
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Affichage d'un message clair si la connexion Supabase échoue */}
      {errorMessage ? (
        <Card className="border-destructive bg-destructive/5">
          <CardContent className="pt-6 text-sm text-destructive font-mono">
            ⚠️ Erreur de liaison Supabase : {errorMessage}
            <br />
            <span className="text-xs text-gray-500 block mt-2">
              Vérifiez vos variables NEXT_PUBLIC_SUPABASE_URL et KEY dans votre fichier .env.local
            </span>
          </CardContent>
        </Card>
      ) : (
