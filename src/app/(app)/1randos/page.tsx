...
useEffect(() => {
  setMounted(true);
  fetch('/api/1randos') // On appelle la bonne route
    .then(res => res.json())
    .then(json => {
...
useEffect(() => {
  ...
  const endpoint = randoInfo 
    ? `/api/1randos?cat=${encodeURIComponent(randoInfo.cat)}&file=${encodeURIComponent(randoInfo.file)}`
    : `/api/1randos/${selectedSource}`; // Cas Supabase/ID simple
