...
function CreateEventFormRando5() {
  ...
  // Récupération des paramètres de l'URL (?source=rando_Lot&id=Nom_Rando)
  const id = searchParams.get('id');
  const sourceParam = searchParams.get('source') || 'rando_Lot'; 
  ...
  useEffect(() => {
    ...
    fetch(`/api/rando_Lot?rando=${encodeURIComponent(id)}`)
      .then(res => res.json())
      .then(data => {
