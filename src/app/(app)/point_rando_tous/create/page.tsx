...
function CreateEventFormRando11() {
  ...
  const sourceParam = searchParams.get('source') || 'point_rando_tous'; 
  
  const [filteredCircuits, setFilteredCircuits] = useState<any[]>([]);
  const [currentCategory, setCurrentCategory] = useState<string>("Occitanie");
  ...
  // RÉCUPÉRATION DE L'ITINÉRAIRE DEPUIS L'API RANDO11BALRAND
  useEffect(() => {
    fetch('/api/point_rando_tous')
      ...
      .then(data => {
        if (Array.isArray(data) && id) {
          ...
          if (found) {
            ...
            setCurrentCategory(found.categorie || "Occitanie");
            setTitle(`Randonnée : ${found.nom}`);
            setMeetingPoint(found.commune || found.depart || "");
      ...
      .catch(err => {
        console.error("Erreur lors du chargement du circuit rando11balrand:", err);
        setLoading(false);
      });
  }, [id]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8 bg-slate-50/40 min-h-screen">
      <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">
        Planifier une Sortie <span className="text-indigo-600">{currentCategory}</span>
      </h1>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="border-b bg-slate-100/50">
          <CardTitle className="flex items-center gap-2 text-lg text-slate-900">
            <ClipboardList className="w-5 h-5 text-indigo-600"/> Description de la randonnée
          </CardTitle>
        </CardHeader>
      ...
      <Card className="border-slate-200 shadow-sm overflow-hidden">
        <CardHeader className="bg-slate-900 text-slate-50">
          <CardTitle className="flex items-center gap-2 text-md">
            <Map size={18} className="text-indigo-400"/> Localisation du sentier sélectionné
          </CardTitle>
        </CardHeader>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-900 p-8 rounded-2xl shadow-xl mt-10">
        <div className="space-y-1">
          <h4 className="text-xl font-bold text-indigo-400">Publier la sortie ?</h4>
